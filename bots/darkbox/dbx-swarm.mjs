...
async function recenter(bot, side, prob) {
  ...
  // Only force a big frontier move to OPEN the book from the construction tick
  // (200000). Once it's in-band we follow the market — reposting around the live
  // currentTick — instead of fighting it with moveTickTo, which reverts when it
  // would cross resting liquidity. Price drift then comes from taker flow.
  let cur = Number(await pub.readContract({ address: book, abi: bookAbi, functionName: "currentTick" }));
  if (Math.abs(cur - target) > 4000) {
    try {
...
// ── taker actions ─────────────────────────────────────────────────────--
let bias = 0; // slow random walk so price drifts and the chart moves
async function takerTrade(bot) {
  bias = Math.max(-1, Math.min(1, bias + (Math.random() - 0.5) * 0.5));
  const side = Math.random() < 0.6 ? "YES" : "NO";
  const { book, token } = BOOKS[side];
  const buy = Math.random() < 0.5 + 0.28 * bias;
...
async function main() {
  ...
  if (cfg.live) {
    ...
    if (!cfg.noMm) {
      await recenter(mmYes, "YES", cfg.fair);
      await recenter(mmNo, "NO", 1 - cfg.fair);
    }
    report();
    if (cfg.fundOnly) {
  ...
  if (cfg.live && !cfg.noMm) {
    let fairWalk = cfg.fair;
    mmTimer = setInterval(async () => {
      if (stopping) return;
      fairWalk = Math.max(0.08, Math.min(0.92, fairWalk + (Math.random() - 0.5) * 0.04));
      try {
        await recenter(mmYes, "YES", fairWalk);
        await recenter(mmNo, "NO", 1 - fairWalk);
      } catch (e) {
