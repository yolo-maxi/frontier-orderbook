# Copy Liquidity — Design Debate

A threaded, adversarial review of the copy-liquidity experiment. Two reviewers
...
back half of an order, the copy half fills at the real touch instead.
...
30 bps copy fee on the mirrored leg.
...
you can hit it*; copy depth is the opposite — it only materializes *at the
...
`+copy` tag and a pooled-inventory legend — see `OrderBook.tsx`. On the fee:
yes, the taker pays full freight on the copy leg. That is deliberate (Thread B),
and the taker still comes out ahead versus walking a thin book, which is the
alternative when copy is the only incremental depth available.
...
Copy flow is *incremental* — it's the size the real book couldn't serve at that
...
collapse into each other: copy can't post first, can't set price, earns no
maker treatment — so no maker rationally defects into being a copy LP to dodge
...
Copy LPs earn the book spread without ever quoting, repricing, or bearing
...
**Maintainer.** The alignment argument holds *only* because copy earns no maker
...
for copy). The cannibalization risk is real but its magnitude is dominated by
...
first and copy only ever takes overflow the makers *couldn't* serve — at which
point makers lose nothing and the free-riding reduces to "copy earns spread on
...
## Thread C — Are copy LPs a sacrificial, adversely-selected book?

**Bear.** This is the core flaw. The mirror fires on *every* sweep, 1:1, at the
print, and the LP cannot pull, reprice, or decline. So copy LPs are forced to
...
**Bull.** Compared to a vanilla AMM, copy LPs are *better* off: in an AMM the LP
is always the marginal counterparty; here a real maker is filled first and the
copy LP only takes the residual at a price a competing maker independently
...
for the experiment and state it plainly: **copy LPs are taking a directional
...
should explore (a) sharing part of the copy fee back to LPs as risk
...
real resting depth, and/or cap mirrored notional per block. Until then, copy
...
quotes a market with no flow, no taker comes to a market with no depth. Copy
...
copy liquidity is a **cold-start tool, not a hold-to-resolution position.**
Sponsors should withdraw copy inventory as a market approaches resolution, and a
...
(1) `grossBudget/2` halves the real fill *whenever any copy inventory exists*,
...
- **(3) Rounding — mitigated.** The mirror uses `_mulDivUp` for the taker's input
  ...
  fuzz/invariant pass dedicated to copy rounding is warranted before production.
...
Copy liquidity is a clean, oracle-free way to bootstrap depth and to convert
...
copy LPs (Thread C), thin-book manipulability for lack of a min-depth gate
