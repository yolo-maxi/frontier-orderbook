# Mirror Liquidity — Design Debate

A threaded, adversarial review of the mirror-liquidity experiment. Two reviewers
...
(`UniformFrontierBook.sol`, `UniformMakerOps.sol`, see `MIRROR_LIQUIDITY.md` for mechanics).
...
back half of an order, the mirror half fills at the real touch instead.
...
30 bps mirror fee on the mirrored leg.
...
you can hit it*; mirror depth is the opposite — it only materializes *at the
...
`+mirror` tag and a pooled-inventory legend — see `OrderBook.tsx`. On the fee:
yes, the taker pays full freight on the mirror leg. That is deliberate (Thread B),
and the taker still comes out ahead versus walking a thin book, which is the
alternative when mirror is the only incremental depth available.
...
Mirror flow is *incremental* — it's the size the real book couldn't serve at that
...
benign mirror and subsidize the price-setting original. The two tiers can't
collapse into each other: mirror can't post first, can't set price, earns no
maker treatment — so no maker rationally defects into being a mirror LP to dodge
...
Mirror LPs earn the book spread without ever quoting, repricing, or bearing
...
**Maintainer.** The alignment argument holds *only* because mirror earns no maker
treatment and fills strictly second — both true in code (`_mirrorAsk` /
`_mirrorBid` run after `_sweepUp`/`_sweepDown`, and there is no rebate path
for mirror). The cannibalization risk is real but its magnitude is dominated by
...
first and mirror only ever takes overflow the makers *couldn't* serve — at which
point makers lose nothing and the free-riding reduces to "mirror earns spread on
...
## Thread C — Are mirror LPs a sacrificial, adversely-selected book?

**Bear.** This is the core flaw. The mirror fires on *every* sweep, 1:1, at the
print, and the LP cannot pull, reprice, or decline. So mirror LPs are forced to
...
**Bull.** Compared to a vanilla AMM, mirror LPs are *better* off: in an AMM the LP
is always the marginal counterparty; here a real maker is filled first and the
mirror LP only takes the residual at a price a competing maker independently
...
for the experiment and state it plainly: **mirror LPs are taking a directional
inventory bet financed by spread capture, and must actively manage exposure by
withdrawing** (`withdrawMirror` is pro-rata and always available). Productionizing
should explore (a) sharing part of the mirror fee back to LPs as risk
...
real resting depth, and/or cap mirrored notional per block. Until then, mirror
...
quotes a market with no flow, no taker comes to a market with no depth. Mirror
...
mirror liquidity is a **cold-start tool, not a hold-to-resolution position.**
Sponsors should withdraw mirror inventory as a market approaches resolution, and a
...
(1) `grossBudget/2` halves the real fill *whenever any mirror inventory exists*,
...
- **(1) Budget halving — accepted shortcut, highest-impact fix.** Real and worth
  fixing first: it should be `min(mirrorReserves, residualBudget)` *after* the
  real book gets full budget, not a blind 50/50. Documented in `MIRROR_LIQUIDITY.md`.
...
- **(3) Rounding — mitigated.** The mirror uses `_mulDivUp` for the taker's input
  ...
  fuzz/invariant pass dedicated to mirror rounding is warranted before production.
- **(4) token1-settled fee — mitigated, not a liability.** On an ask mirror the
  ...
  is capped at `mirrorReserve1` *before* the fee is carved, so the pool can always
  ...
  capped away.
- **(5) First-depositor ratio — accepted, low severity here.** `depositMirror`
...
Mirror liquidity is a clean, oracle-free way to bootstrap depth and to convert
...
mirror LPs (Thread C), thin-book manipulability for lack of a min-depth gate
