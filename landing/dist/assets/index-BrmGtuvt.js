var b=Object.defineProperty;var k=(t,e,i)=>e in t?b(t,e,{enumerable:!0,configurable:!0,writable:!0,value:i}):t[e]=i;var h=(t,e,i)=>k(t,typeof e!="symbol"?e+"":e,i);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function i(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(a){if(a.ep)return;a.ep=!0;const r=i(a);fetch(a.href,r)}})();const w=[{id:"range",eyebrow:"Ladder orders",title:"Many makers' orders, one clean book.",summary:"Each maker posts a ladder — limit orders spread across the prices they pick. Frontier stacks every overlapping order into one depth per price, so takers see a single, clean book.",caption:"Gold bands are individual ladders, one per address, each spanning its own price range. Green is the combined depth at each price — the size every taker actually fills against. Many makers in, one book out.",bullets:["Each address posts its own ladder across any range of prices.","Overlapping orders combine into one depth per price.","Takers fill against the combined depth, never individual orders."],svg:`
...
</svg>`},{id:"fill",eyebrow:"Getting filled",title:"One market order, every maker in its path paid.",summary:"A market order fills from the best price inward, taking depth price by price until your size runs out. Because that depth is many makers stacked together, one fill pays every maker it crosses — in a single trade.",caption:"Red is the depth your market order takes, filled from the best price inward and stopping where your size runs out. Every maker whose orders sit in that range is paid at once (✓); makers beyond the fill are untouched.",bullets:["Takers fill against the combined depth, never individual orders.","One fill pays every maker in its price path, pro-rata.","The fill stops at the price where your size runs out."],svg:`
...
</svg>`},{id:"mirror",eyebrow:"Mirror liquidity",title:"Back the book without managing orders.",summary:"Your resting bids and asks are the base layer. Mirror liquidity stacks on top of every bar in gold, adding depth across the book — and when a trade moves the price or a new maker shows up, it follows them instantly.",caption:"Green and red are resting depth; gold is mirror liquidity stacked on top. Watch it follow a trade across the book, then mirror a new maker the moment they quote.",bullets:["Mirror liquidity adds depth on top of every resting bar.","When a trade moves the price, your mirror depth follows it across the book.","A new maker gets mirrored the instant they quote — no bot, no requoting."],svg:`
...
`},{id:"sweep",eyebrow:"Settlement",title:"Fill thousands of orders. Everyone gets paid at once.",summary:"Liquidity is orders stacked from many owners into one book. A market order fills straight up through the stack, and every order it crosses settles together in a single trade.",caption:"Each owner's proceeds wait at their own address — no bot, no expiry. Filling three orders or three thousand settles the same way.",bullets:["Stacked orders from independent owners form one book.","One market order fills everything it touches in a single trade.","Proceeds wait at each owner's address until they claim."],svg:`
...
</svg>`}],y=window.matchMedia("(prefers-reduced-motion: reduce)").matches,v=[["Mechanism","#mechanism"],["Ladders","#range-orders"],["Mirror","#mirror-liquidity"],["Markets","#markets"],["Build","#build"]],g=document.querySelector("#app");if(!g)throw new Error("Missing #app root");function M(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const S=[{id:"perm-grant",label:"Grant",group:"permissions",lines:["// Let a market-making bot move your quotes for 7 days — nothing else.","// It can requote, but it can never withdraw your funds.","bytes4[] memory allowed = new bytes4[](2);","allowed[0] = book.requote.selector;     // move your asks","allowed[1] = book.requoteBid.selector;  // move your bids","","permissions.grantSelectorBundle(","    botOperator,                       // who you trust","    address(book),                     // on which book","    allowed,                           // exactly these calls","    uint48(block.timestamp + 7 days)   // auto-expires",");"]},{id:"perm-revoke",label:"Revoke",group:"permissions",lines:["// Cut a bot off instantly — every selector, in one call.","permissions.revokeAll(botOperator, address(book));","","// …or narrow it down to a single power:","permissions.revoke(","    botOperator,","    address(book),","    book.requoteBid.selector   // can no longer touch your bids",");"]},{id:"hook-oracle",label:"Volume oracle",group:"hooks",lines:["// A hook attaches your own logic to the book. This one keeps a","// running volume oracle, updated on every fill — no keeper needed.","contract VolumeOracle is IFrontierHooks {","    uint256 public totalVolume;","","    function afterSweep(address taker, int24 fromTick, int24 reached,","        uint256 paid, uint256 received) external returns (bytes4) {","        totalVolume += received > paid ? received : paid;","        return IFrontierHooks.afterSweep.selector;","    }","}"]},{id:"hook-allow",label:"Maker allowlist",group:"hooks",lines:["// Gate who may post liquidity — revert in beforeDeposit to block a","// maker. Same shape gives KYC’d venues, whitelists, compliance.","contract MakerAllowlist is IFrontierHooks {","    mapping(address => bool) public allowed;","","    function beforeDeposit(address maker, int24, int24, uint128, int128, bool)","        external view returns (bytes4) {",'        require(allowed[maker], "maker not allowed");',"        return IFrontierHooks.beforeDeposit.selector;","    }","}"]},{id:"hook-breaker",label:"Circuit breaker",group:"hooks",lines:["// Stop a violent move before it lands: revert in beforeSweep if one","// order would push the price past your guardrail.","contract CircuitBreaker is IFrontierHooks {","    int24 public maxMove;","","    function beforeSweep(address taker, int24 fromTick, int24 target)","        external view returns (bytes4) {","        int24 move = target > fromTick ? target - fromTick : fromTick - target;",'        require(move <= maxMove, "circuit breaker tripped");',"        return IFrontierHooks.beforeSweep.selector;","    }","}"]}],T=new Set(["contract","is","function","external","public","private","internal","view","pure","payable","returns","return","memory","storage","calldata","require","new","emit","mapping","if","else","for","while","using","override","virtual","constant","immutable"]),C=/^(?:u?int\d*|address|bool|bytes\d*|string)\b/,N=/^[A-Za-z_$][\w$]*/;function E(t){let e=t,i="",s="";const a=(r,l)=>{const o=M(l);s+=r?`<span class="tok-${r}">${o}</span>`:o};for(;e.length;){if(e.startsWith("//")){a("comment",e);break}const r=/^"(?:[^"\\]|\\.)*"/.exec(e);if(r){a("str",r[0]),e=e.slice(r[0].length),i='"';continue}const l=C.exec(e);if(l){a("type",l[0]),e=e.slice(l[0].length),i=l[0].slice(-1);continue}const o=N.exec(e);if(o){const d=o[0],p=e.slice(d.length);let x="";T.has(d)?x="kw":d==="true"||d==="false"?x="num":i==="."?x="prop":/^\s*\(/.test(p)&&(x="fn"),a(x,d),e=p,i=d.slice(-1);continue}const n=/^\d[\w]*/.exec(e);if(n){a("num",n[0]),e=e.slice(n[0].length),i=n[0].slice(-1);continue}const c=/^\s+/.exec(e);if(c){s+=c[0],e=e.slice(c[0].length);continue}const f=e[0];a("punct",f),e=e.slice(1),i=f}return s||"&nbsp;"}function A(t){let e="";const i=t.map((a,r)=>{const l=e&&e!==a.group?'<span class="cw-sep" aria-hidden="true"></span>':"";e=a.group;const o=`<span class="cw-tab-group">${a.group==="permissions"?"PERMISSIONS":"HOOKS"}</span>`,n=r===0||t[r-1].group!==a.group;return`${l}<button class="cw-tab${r===0?" is-active":""}" type="button" role="tab" data-cw-tab="${a.id}">${n?o:""}${a.label}</button>`}).join(""),s=t.map((a,r)=>{const l=a.lines.map(o=>`<span class="code-line">${E(o)}</span>`).join("");return`<pre class="cw-pane${r===0?" is-active":""}" data-cw-pane="${a.id}"><code>${l}</code></pre>`}).join("");return`
    <div class="code-window" data-reveal>
      <div class="cw-bar">
        <span class="cw-dots" aria-hidden="true"><i></i><i></i><i></i></span>
        <div class="cw-tabs" role="tablist" aria-label="Permissions and hooks examples">${i}</div>
      </div>
  ...
  <main id="main">
    <section id="top" class="hero" aria-labelledby="hero-title">
      ...
      <div class="hero-stats" data-reveal>
        ${u("214,805","gas to fill any market order","214805")}
        ${u("Same fee","across 5 or 5,000 prices filled")}
        ${u("1/10¢","the smallest price step")}
      </div>
    </section>
    ...
    ${w.filter(t=>t.id!=="sweep").map($).join("")}
...
`;function m(t){return(t==="ask"?[["4,001.20","18.42",74],["4,000.80","12.10",54],["4,000.40","22.09",88]]:[["3,999.60","16.30",64],["3,999.20","26.44",94],["3,998.80","14.87",58]]).map(([i,s,a])=>`
        <div class="book-row book-row--${t}">
          <span>${i}</span>
          <span>${s}</span>
          <i style="--depth:${a}%"></i>
        </div>`).join("")}function u(t,e,i,s,a){return`
    <div class="stat">
      <strong class="mono" ${i?`data-count="${i}"`:""}>${i?"0":t}</strong>
      <span>${e}</span>
      
    </div>
  `}function $(t){const e=t.id==="range"?"range-orders":t.id==="fill"?"taker-fill":t.id==="mirror"?"mirror-liquidity":"frontier-sweep",i=t.id==="mirror"?" feature-section--reverse":"",s=`<button class="replay-button" type="button" data-replay="${t.id}">Replay</button>`;return`
    <section id="${e}" class="section feature-section${i}" aria-labelledby="${e}-title">
      <div class="feature-copy" data-reveal>
        <p class="section-kicker">${t.eyebrow}</p>
        <h2 id="${e}-title">${t.title}</h2>
        <p>${t.summary}</p>
        <ul>
          ${t.bullets.map(a=>`<li>${a}</li>`).join("")}
        </ul>
      </div>
      <div class="explainer-card" data-explainer="${t.id}" data-reveal>
  ...
  `}function O(){const t=[...document.querySelectorAll("[data-reveal], .explainer-card")];if(!("IntersectionObserver"in window)){t.forEach(i=>i.classList.add("is-visible"));return}const e=new IntersectionObserver(i=>{for(const s of i)s.isIntersecting&&(s.target.classList.add("is-visible"),e.unobserve(s.target))},{threshold:.18,rootMargin:"0px 0px -8% 0px"});t.forEach(i=>e.observe(i))}function L(){const t=[...document.querySelectorAll("[data-count]")],e=s=>{const a=Number(s.dataset.count),r=s.dataset.suffix??"",l=s.dataset.compact==="true",o=performance.now(),n=y?0:900,c=f=>{const d=n===0?1:Math.min(1,(f-o)/n),p=1-Math.pow(1-d,3),x=a*p;s.textContent=l?q(x):Math.round(x).toLocaleString("en-US")+r,d<1&&requestAnimationFrame(c)};requestAnimationFrame(c)};if(!("IntersectionObserver"in window)){t.forEach(e);return}const i=new IntersectionObserver(s=>{for(const a of s)a.isIntersecting&&(e(a.target),i.unobserve(a.target))},{threshold:.5});t.forEach(s=>i.observe(s))}function q(t){return t>=1e6?`${(t/1e6).toFixed(1)}M`:t>=1e3?`${Math.round(t).toLocaleString("en-US")}`:`${Math.round(t)}`}function R(){document.querySelectorAll("[data-replay]").forEach(t=>{t.addEventListener("click",()=>{const e=t.closest(".explainer-card");e&&(e.classList.remove("is-visible","is-replaying"),e.offsetWidth,e.classList.add("is-visible","is-replaying"),window.setTimeout(()=>e.classList.remove("is-replaying"),2600))})})}function F(){document.querySelectorAll("[data-cw-tab]").forEach(t=>{t.addEventListener("click",()=>{const e=t.getAttribute("data-cw-tab"),i=t.closest(".code-window");!i||!e||(i.querySelectorAll("[data-cw-tab]").forEach(s=>s.classList.toggle("is-active",s===t)),i.querySelectorAll("[data-cw-pane]").forEach(s=>s.classList.toggle("is-active",s.getAttribute("data-cw-pane")===e)))})})}function I(){const t=document.querySelector(".site-header");if(!t)return;const e=()=>t.classList.toggle("is-scrolled",window.scrollY>20);e(),window.addEventListener("scroll",e,{passive:!0})}class D{constructor(e){h(this,"canvas");h(this,"ctx");h(this,"dpr",Math.min(window.devicePixelRatio||1,2));h(this,"width",0);h(this,"height",0);h(this,"tick",0);h(this,"pointer",{x:.5,y:.5});h(this,"particles",Array.from({length:52},(e,i)=>({x:i*97%1e3,y:i*173%1e3,r:.7+i%4*.35,s:.15+i%7*.035})));const i=e.getContext("2d");if(!i)throw new Error("Canvas 2D context unavailable");this.canvas=e,this.ctx=i,this.resize(),window.addEventListener("resize",()=>this.resize()),window.addEventListener("pointermove",s=>{this.pointer={x:s.clientX/Math.max(1,this.width),y:s.clientY/Math.max(1,this.height)}},{passive:!0})}start(){this.draw(),y||requestAnimationFrame(()=>this.loop())}loop(){this.tick+=1,this.draw(),requestAnimationFrame(()=>this.loop())}resize(){this.width=window.innerWidth,this.height=window.innerHeight,this.canvas.width=Math.floor(this.width*this.dpr),this.canvas.height=Math.floor(this.height*this.dpr),this.canvas.style.width=`${this.width}px`,this.canvas.style.height=`${this.height}px`,this.ctx.setTransform(this.dpr,0,0,this.dpr,0,0),this.draw()}draw(){const e=this.ctx;e.clearRect(0,0,this.width,this.height),e.fillStyle="#0b0e11",e.fillRect(0,0,this.width,this.height),this.drawGrid(e),this.drawDepth(e),this.drawParticles(e)}drawGrid(e){e.save(),e.globalAlpha=.18,e.strokeStyle="#2ebd85",e.lineWidth=1;const i=y?0:this.tick*.18%48;for(let s=-48+i;s<this.width+48;s+=48)e.beginPath(),e.moveTo(s,0),e.lineTo(s,this.height),e.stroke();for(let s=-48+i;s<this.height+48;s+=48)e.beginPath(),e.moveTo(0,s),e.lineTo(this.width,s),e.stroke();e.restore()}drawDepth(e){const i=this.width*(.68+(this.pointer.x-.5)*.025),s=this.height*.5,a=12;e.save(),e.globalAlpha=.5;for(let l=0;l<a;l+=1){const o=s-230+l*42,n=l<a/2?"ask":"bid",c=88+(l*37+this.tick)%150;e.fillStyle=n==="ask"?"rgba(246,70,93,.28)":"rgba(46,189,133,.28)",e.fillRect(i-c/2,o,c,12)}const r=i+Math.sin(this.tick/80)*24;e.globalAlpha=.82,e.strokeStyle="#2ebd85",e.lineWidth=2,e.beginPath(),e.moveTo(r,s-260),e.lineTo(r,s+260),e.stroke(),e.restore()}drawParticles(e){e.save();for(const i of this.particles){const s=(i.x+this.tick*i.s*7)%1e3/1e3*this.width,a=(i.y+this.tick*i.s*3)%1e3/1e3*this.height,r=Math.max(0,1-Math.hypot(s-this.pointer.x*this.width,a-this.pointer.y*this.height)/320);e.globalAlpha=.2+r*.32,e.fillStyle=i.r>1.4?"#f0b90b":"#8a93a0",e.beginPath(),e.arc(s,a,i.r+r,0,Math.PI*2),e.fill()}e.restore()}}function P(){const t=document.querySelector("#market-field");t&&new D(t).start()}O();L();R();F();I();P();
