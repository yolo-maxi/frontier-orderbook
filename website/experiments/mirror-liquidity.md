...
**Answer:** yes; built on the uniform book, 12 tests passing. Mirror liquidity
is a single pooled inventory that **mirrors real fills at the book price**,
pays the protocol mirror fee when the book has a fee recipient, and earns no
maker treatment.

## What it is

A taker crossing the book hits resting maker orders. With mirror liquidity live,
...
book's price. The mirror pool never posts a level of its own; it has no opinion
on price. It only echoes what real price discovery already produced.

LPs add inventory with `depositMirror` and receive pro-rata shares (an AMM-style
pool, oracle-free; the first deposit sets the ratio). They earn the book spread
on mirrored size and bear inventory risk, exactly like the maker they mirror.

## The fee twist — why honest makers come out ahead

Every mirror-mirrored fill pays a fee (30 bps in the experiment) on the quote
leg when `feeRecipient` is set, routed to the **protocol**, never retained
by the pool. With no fee recipient, the mirror fee is zero and the mirror
settles at pure book price. Mirror depth is explicitly *not* a maker: no
maker-fee waiver, no rebate eligibility. If the book has a nonzero taker
fee, the taker also pays it on the combined real+mirror notional.

The intuition: mirror volume is extra protocol revenue from flow that would not
...
dead before it has a chance. Mirror liquidity lets a sponsor seed one pooled
...
seeded mirror depth shows up as a hatched extension on each order-book level,
visually distinct from real resting orders.

## Seeing it in the demo

Open the **Mirror** tab to add or withdraw pooled inventory and watch the
order book: real depth is the solid bar, mirror-mirrored depth is the hatched
segment to its left, capped by pooled reserves, with a `+mirror` tag on each
...
budget-halving between real and mirror fills (capital-inefficient when mirror
inventory is small), a single global pool with no price view, and a constant
fee rate. The case *against* the design — adverse selection on mirror LPs,
...
project's design debate (`prototype/MIRROR_LIQUIDITY_DEBATE.md`), alongside the mechanics
write-up (`prototype/MIRROR_LIQUIDITY.md`).
