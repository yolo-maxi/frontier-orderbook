...
- Behavioral tests sampled: gas, hooks, fees, permissions, periphery, position NFT, shadow/copy liquidity, geometric book, RangeLP, YieldRangeLP.
...
Gas rows in the website table were refreshed from `forge test --isolate -vv` runs on `FrontierGasTest`, `GasMatrixTest`, `GeoBookTest`, `PublishBenchTest`, `FrontierShadowTest`, and `FrontierVenueTest`. TWAP hook overhead was refreshed from `HookExperimentsTest`: 31,770 gas on the first sweep, 34,550 gas steady-state.
...
| `guide/mechanism.md` | Claimed internal balances and `recycleBidIntoAsk` still existed. | Replaces that with current shadow/copy-liquidity accounting and per-book fee fields. | `FrontierBookBase`, `UniformFrontierBook`, `UniformMakerOps`, `FrontierRecycle.t.sol`. |
...
| `guide/gas.md` | Prose and table used old gas numbers, removed shaped/recycle rows, and old `FrontierMakerOps` name. | Table and prose use current gas logs and current `UniformMakerOps` / `GeometricMakerOps` names. | `FrontierGasTest`, `GasMatrixTest`, `PublishBenchTest`, `GeoBookTest`, `FrontierShadowTest`. |
...
| `experiments/copy-liquidity.md` | Claimed every copy fill always pays 30 bps and takers always pay taker fees. | Explains 30 bps copy fee applies only when `feeRecipient` exists, no-recipient books mirror fee-free, and taker fee applies only if configured. | `UniformFrontierBook._shadowFee`, `FrontierShadow.t.sol`. |
...
| `GasCostTable.vue` | Embedded stale gas data and removed shaped/recycle rows. | Replaced with current measured rows for takers, makers, factory deployment, and copy-liquidity pool operations. | `forge test --isolate -vv` logs listed above. |
...
- `forge test --isolate -vv --match-contract "(FrontierGasTest|GasMatrixTest|GeoBookTest|PublishBenchTest|FrontierShadowTest)"`: 36 passed, 0 failed.
